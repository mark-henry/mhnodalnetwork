...
async function initializeDatabase() {
  ...
  const initQuery = `
    // Ensure a clean slate for the demo database
    OPTIONAL MATCH (n) DETACH DELETE n;

    // Create Graph node - this should get ID 0 if the DB is empty.
    CREATE (g:Graph {name: 'Ephemeral Brainstorming Session'})
    // Explicitly set the 'id' property required by the application logic for graphs
    SET g.id = 0; 

    // Create IDCounter (needed by the app's internal ID generation, start high to avoid collision with example nodes)
    MERGE (idc:IDCounter) ON CREATE SET idc.counter = 100 ON MATCH SET idc.counter = 100;

    // Create Example Nodes explaining the app
    ...
    // Assign the 'id' property required by the application logic for nodes
    // Note: We rely on IDCounter starting at 100. The app's makenewid() increments *before* returning.
    // So the first ID generated by the app *after* this setup would be 101.
    // These IDs are distinct from Neo4j's internal IDs but used by the app logic (e.g., in getNode).
    SET welcome.id = 1
    SET features.id = 2
    SET ephemeral.id = 3
    SET tech.id = 4
    ...
    // Create Edges (relationships between nodes) - Using the Node variables directly
    WITH g, welcome, features, ephemeral, tech, you
    ...
    // Associate Nodes with the Graph (ensures they appear in the graph UI) - Using the Node variables directly
    WITH g, welcome, features, ephemeral, tech, you
    ...
    // Return the internal Neo4j ID of the graph we created.
    WITH g
    RETURN id(g) as graphId
  `;
  const session = driver.session({ database: 'neo4j' });
  try {
    // Check if the graph already exists (e.g., if server restarts but DB doesn't)
    const checkResult = await session.run('MATCH (g:Graph {id: 0}) RETURN g LIMIT 1');
    if (checkResult.records.length === 0) {
        console.log('No existing graph found. Running initialization query...');
        const result = await session.run(initQuery);
        const singleRecord = result.records[0];
        const graphId = singleRecord.get('graphId');
        console.log(`Database initialized. Created graph with internal ID: ${graphId}. Hash ID: ${hashids.encode(graphId.low)}`);
         // Verify IDCounter
    ...
    // Decide if server should proceed or exit? For now, log and continue.
    // Potentially throw error here to prevent server start if DB init is critical
  } finally {
    await session.close();
